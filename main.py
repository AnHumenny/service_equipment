...
class Win(tk.Tk):
    def __init__(self, *args, **kwargs):
      ...
      #  self.resizable(False, False)   #изменение размера
        ...
        self.frame_info = tk.Frame(self, width=800)
    ...
    dtb = config.SqlEquipment(host=os.getenv('host'), port=os.getenv('port'), user=os.getenv('user'),
                              password=os.getenv('password'), dtb=os.getenv('dtb'))
    ...
    def add_new_user(self):
        ...
        check_new_login = self.dtb.check_login(login)
        if len(password) < 8:
        ...
        if check_new_login is True:
            ...
            self.dtb.insert_new_user(name, login, password, status)
            name_user = name
            for widget in self.frame_info_user.winfo_children():
                widget.update()
    ...
    def dlt_user(self):
        ...
        self.dtb.delete_user(dlt_user)
        self.del_user()
        
    def all_users(self):
        result = self.dtb.select_all_users()
        for widget in self.frame_info.winfo_children():
            widget.destroy()
        label_user = tk.Text(self.frame_info, width=126, height=46)
        for rows in result:
    ...
    def check_user(self):
        ...
        pass_word = password.encode('utf-8')
        password = base64.b64encode(pass_word)
        result = self.dtb.check_admin(login, password)
        result_s_admin = self.dtb.check_superadmin(login, password)
        if result is True:
    ...
    def select_equipment(self):
        ...
        if self.authent_OK is False:
            ...
            if equip is None:
                ...
                for widget in self.frame_info.winfo_children():
                    widget.destroy()
            if self.dtb.select_equipment_result(equip) == '':
                for widget in self.frame_info.winfo_children():
                    widget.destroy()
            else:
                res_equip = self.dtb.select_equipment_result(equip)
    ...
    def name_equipment(self):
        ...
        if self.authent_OK is False:
            ...
            if name == '':
                ...
                res_name_equipment = self.dtb.select_equipment_name(name)
                scrollbar = tk.Scrollbar(self.frame_info)
                scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
                my_list = tk.Text(self.frame_info, width=126, height=46,
                                  wrap=WORD, yscrollcommand=scrollbar.set)
    ...
    def adresses_equipment(self):
        ...
        if self.authent_OK is False:
            ...
            if address == '':
                ...
                result_address = self.dtb.select_adresses_equipment(address)
                scrollbar = tk.Scrollbar(self.frame_info)
                scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
                my_list = tk.Text(self.frame_info, width=126, height=46,
                                  wrap=WORD, yscrollcommand=scrollbar.set)
    ...
    def add_equip(self):
        ...
        for key, value in result_add.items():
            my_list = tk.Label(self.frame_info, width=126)
            my_list['text'] = value
            my_list.pack()
    ...
    def select_ascue(self):
        ...
        if self.authent_OK is False:
            ...
            if self.dtb.select_ascue_all() is None:
                self.select_ascue()
            else:
                result_ascue = self.dtb.select_ascue_all()
                scrollbar = tk.Scrollbar(self.frame_info)
                scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
                my_list = tk.Text(self.frame_info, width=126, height=46,
                                  wrap=WORD, yscrollcommand=scrollbar.set)
    ...
    def searth_street_without_namber(self):
        ...
        if self.authent_OK is False:
            ...
            street = self.shared_ascue.get()
            if self.dtb.searth_ascue_street(street) == '':
                self.start()
            else:
                result_without_number = self.dtb.searth_ascue_street(street)
                scrollbar = tk.Scrollbar(self.frame_info)
                scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
                my_list = tk.Text(self.frame_info, width=126, height=46,
                                  wrap=WORD, yscrollcommand=scrollbar.set)
    ...
    def search_ip(self):
        ...
        if self.authent_OK is False:
            ...
            result_ip = self.dtb.search_ascue_ip(ip)
            my_list = tk.Text(self.frame_info, width=126, height=46, wrap=WORD)
            try:
    ...
    def ascue_del(self):
        ...
        if self.authent_OK is False:
            ...
            result_del = self.dtb.select_ascue_del()
